 ...
 * 奚童分析阅读质量评估 — depth=2
 *
 * 验证：有书籍上下文时，奚童能深度分析具体概念、引用章节、生成 wiki 链接
 * 路由预期：S0 Router → S1 Inspectional → S2-Pre → S2 Analytical → S4 Formatter
 * 工具调用预期：search_book, read_book_section
 * 前置：插件加载 + API Key + 纳瓦尔宝典完整索引（tree.json + chunks.jsonl + vectors.jsonl）
 */
...
// ---------------------------------------------------------------------------
// 六维评分函数（depth=2 版本，REF 维度要求 wiki 链接）
// ---------------------------------------------------------------------------
		...
		for (const tc of TEST_CASES) {
			await step(`Q: "${tc.question}"`, async () => {
				...
				if (tc.minLength && response.length < tc.minLength) {
					throw new Error(
						`回复长度 ${response.length} 低于要求 ${tc.minLength}: ${response.slice(0, 200)}`
					);
				}
				...
				if (qa.details.wikiPlainLinks.length > 0) {
					lines.push(`  wiki 链接: ${qa.details.wikiPlainLinks.length} 个`);
				}

				// wiki 链接检查（depth=2 强制要求）
				if (tc.requireWikiLinks && qa.scores.REF === 0) {
