 ...
 * CDP E2E 测试运行器
 *
 * 通过 obsidian CLI 操作已运行的 Obsidian 实例。
 *
 * 用法:
 *   node tests/e2e-cdp/run.mjs                        # 运行全部
 *   node tests/e2e-cdp/run.mjs --only reading-mode    # 指定 spec
 *   node tests/e2e-cdp/run.mjs --verbose              # 详细输出
 */
...
async function main() {
	...
	console.log(`\n${c.bold('🧪 CDP E2E 测试')}${c.gray(` (${specs.length} 个 spec)`)}\n`);
