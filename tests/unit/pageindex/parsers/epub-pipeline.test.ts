...
function extractBlockIds(content: string): string[] {
	return (content.match(/\^([a-zA-Z0-9_-]+)/g) || []).map((b) =>
		b.replace(/^\^/, ""),
	);
}
...
describe("B1-a: EPUB structure parsing", () => {
	...
	it("should have coverImage for well-formed EPUBs", () => {
		...
		// Skip if no EPUB files available (e.g., in CI environment)
		if (booksWithCovers.length === 0) {
			console.log("  ⚠️ Skipping cover image test: no EPUB files available");
			return;
		}
	...
	it("chapters should be ordered starting from 0 or 1 (may skip items)", () => {
		...
		for (const book of books) {
			...
			for (let i = 1; i < book!.chapters.length; i++) {
				expect(book!.chapters[i].order).toBeGreaterThan(
					book!.chapters[i - 1].order,
				);
			}
		}
	});
...
describe("B1-b: Markdown conversion quality", () => {
	it("should generate block IDs for paragraphs", () => {
		...
		const chaptersWithBlocks = AI极简经济学.chapters.filter(
			(ch) => ch.blocks && ch.blocks.length > 0,
		);
		expect(chaptersWithBlocks.length).toBeGreaterThan(
			AI极简经济学.chapters.length * 0.7,
		);
	});
	...
	it("should handle images with proper Markdown syntax", () => {
		...
		const imgChapters = 小岛经济学.chapters.filter((c) =>
			/!\[/.test(c.content),
		);
...
describe("B1-c: Chapter title extraction", () => {
	...
	it("should skip image-only cover pages", () => {
		...
		const imageTitles = AI极简经济学.chapters.filter((c) =>
			c.title.startsWith("!["),
		);
	...
	it("should avoid 'Chapter N' fallback titles for structured EPUBs", () => {
		if (!优秀的绵羊) return;
		const fallbacks = 优秀的绵羊.chapters.filter((c) =>
...
describe("B1-b: Block ID quality", () => {
	...
	it("blockMap should map original HTML anchors to block IDs (优秀的绵羊)", () => {
		...
		const chaptersWithMap = 优秀的绵羊.chapters.filter(
			(c) => c.blockMap && c.blockMap.size > 0,
		);
		...
		for (const book of books) {
			...
			for (const ch of book!.chapters) {
				...
				for (const bid of ch.blocks) {
					...
					expect(
						prev,
						`Duplicate block_id "${bid}" in chapters "${prev}" and "${ch.title}"`,
					).toBeUndefined();
...
describe("Regression: Known edge cases", () => {
	it("AI极简经济学 Part dividers should have meaningful titles (not 'Chapter N')", () => {
		if (!AI极简经济学) return;
		const parts = AI极简经济学.chapters.filter(
			(c) =>
				c.tokenCount < 50 &&
				!c.title.startsWith("Chapter") &&
				/第[一二三四五六七八九十]+部分/.test(c.title),
		);
	...
	it("我看见的世界 should have bilingual chapter structure", () => {
		...
		const chineseTitles = 我看见的世界.chapters.filter((c) =>
			/[\u4e00-\u9fff]/.test(c.title),
		);
		const englishTitles = 我看见的世界.chapters.filter((c) =>
			/^[A-Z]/.test(c.title),
		);
...
describe("Content quality: actual Markdown inspection", () => {
	...
	it("反脆弱 should not have content fragments as chapter titles", () => {
		...
		for (const ch of longTitles) {
			...
			console.log(
				`  Long title (${ch.title.length} chars): "${ch.title.substring(0, 60)}..."`,
			);
		}
	...
	it("content should not contain piracy watermarks", () => {
		...
		if (foundWatermarks > 0) {
			console.log(
				`  ⚠️ Found ${foundWatermarks} watermark(s) — should be filtered`,
			);
		}
	});
	...
	it("我看见的世界: English content chapters should have content", () => {
		...
		const engChapters = 我看见的世界.chapters.filter((c) =>
			/^[A-Z]/.test(c.title),
		);
	...
	it("should not have excessive duplicate H3 headings within a chapter", () => {
		...
		for (const ch of 疯传.chapters) {
			...
			if (h3s.length > 10) {
				...
				if (dupeRatio > 0.3) {
					console.log(
						`  ⚠️ Ch "${ch.title}": ${h3s.length} H3s, ${unique.size} unique (${Math.round(dupeRatio * 100)}% dupes)`,
					);
				}
			}
		}
