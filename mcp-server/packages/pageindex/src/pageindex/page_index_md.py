...
async def get_node_summary(node, summary_token_threshold=200, llm_client=None):
    node_text = node.get("text")
    num_tokens = count_tokens(node_text, model=llm_client.model if llm_client else None)
    if num_tokens < summary_token_threshold:
        return node_text
    else:
        return await generate_node_summary(node, llm_client=llm_client)


async def generate_summaries_for_structure_md(
    structure, summary_token_threshold, llm_client=None
):
    nodes = structure_to_list(structure)
    tasks = [
        get_node_summary(
            node, summary_token_threshold=summary_token_threshold, llm_client=llm_client
        )
        for node in nodes
    ]
...
async def md_to_tree(
    ...
    if if_thinning:
        nodes_with_content = update_node_list_with_text_token_count(
            nodes_with_content, model=llm_client.model if llm_client else None
        )
