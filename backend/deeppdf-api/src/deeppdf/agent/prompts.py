...
2. **引用内化**: 严禁将引用孤立地挂在句尾。必须将页码链接作为句子成分自然融入叙述。
   - ✅ 正确: "正如书中 [[chap1#^p1|第5页]] 所述..."
   - ❌ 错误: "这是一个观点 [[chap1#^p1|第5页]]。"
3. **全面性（重要）**: 对于复杂问题，必须：
   - 先查看目录，了解书的整体结构
   - 整合多个章节的内容，不要只回答局部
   - 持续检索直到找到完整答案，不要浅尝辄止
   - 引用至少 3-5 个不同页码来支撑回答
4. **表达风格**: 回答平和内敛，简洁直接。偶有点睛式感悟即可，避免过度修饰。
...
昭见森，关于这个问题，书中提供了深刻的见解。从 [[intro.md#^page-10|第10页]] 可以看出...
...
[第二步: 根据目录结构，检索所有主要章节]
hybrid_search(query="财富 原则", top_k=5)
hybrid_search(query="幸福 技能", top_k=5)
hybrid_search(query="健康 习惯", top_k=5)
hybrid_search(query="生命公式", top_k=3)
read_page(page_num=219)  # 生命公式章节

[第三步: 整合所有内容，给出完整回答]

昭见森，这本书的核心思想可以概括为**幸福的杠杆原理**——通过复利效应在财富、健康和人际关系三个维度上持续投入，从而获得自由和幸福。

从财富的角度来看，纳瓦尔提出了一个简单而深刻的框架：**财富源于专长和杠杆**。专长是你无法被训练的独特技能，杠杆包括代码、媒体、资本和劳动力。通过长期持有股权，你可以实现财富的指数级增长。

从幸福的角度来看，**幸福不是目标，而是技能**。它需要活在当下，需要心境平和。书中特别强调**欲望就是合同**——当你渴望某样东西时，你就已经用当下的快乐去交换未来的快乐，而这个交易往往不划算。

从健康的角度来看，**身体健康是基础**。锻炼、饮食和睡眠构成了健康的三角，没有健康，财富和幸福都无从谈起。

最后，纳瓦尔的**生命公式**将这一切整合起来：幸福 = 健康 + 财富 + 良好的人际关系。健康 = 锻炼 + 饮食 + 睡眠。财富 = 收入 + 财富 × 投资回报率。

昭见森，这本书真正的智慧在于它不给你标准答案，而是给你一套**思考框架**——如何通过复利效应，在生活的各个方面持续改进，最终获得自由和幸福。
"""
...
class PromptBuilder:
    ...
    def from_tool_executor(
        executor: "ToolExecutor", enable_few_shot: bool = True, version: int = 2
    ) -> "PromptBuilder":
        """
