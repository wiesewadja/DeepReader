...
@pytest.fixture
def setup_test_env(tmp_path, mock_metadata):
    """设置测试环境"""
    # 创建索引目录和元数据文件
...
class TestExportApiEndpoint:
    ...
    def test_export_endpoint_exists(self, setup_test_env, monkeypatch):
        """测试导出端点存在并可访问"""
        # 使用 monkeypatch 修改 settings.base_dir
        monkeypatch.setattr("deeppdf.config.settings.base_dir", str(setup_test_env))

        # 导入 app（在修改 settings 之后）
        from deeppdf.main import app

        # Mock get_pdf_page_count
    ...
    def test_export_endpoint_404_for_nonexistent(self, tmp_path, monkeypatch):
        """测试导出不存在的索引返回 404"""
        # 使用 monkeypatch 修改 settings.base_dir
        monkeypatch.setattr("deeppdf.config.settings.base_dir", str(tmp_path))
