...
4. 【保留 wiki 链接】：analysis 和 structural_analysis 中的 [[...]] 是 Obsidian 双链引用，必须原样保留，不可修改路径、block_id 或别名，也不可丢弃
5. 【禁止编造链接】：只允许保留输入中已有的 [[...]] 链接。绝不可以凭记忆或推测自行创建新的 wiki 链接，即使你确定文件存在。如果输入中没有链接，输出中也不应该有链接
