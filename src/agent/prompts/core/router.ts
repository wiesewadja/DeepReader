...
Output only valid JSON, no Markdown code blocks:
{
  ...
  buildUserMessage: (ctx: Record<string, unknown>): string => {
    const rawQuery = ctx.rawQuery as string;
    const chatHistory = (ctx.chatHistory as Array<{ role: string; content: string }>) || [];
    const bookName = ctx.bookName as string | undefined;
    const docDescription = ctx.docDescription as string | undefined;
    const locale = (ctx.locale as 'zh' | 'en') || 'zh';

    const isZh = locale === 'zh';
    const recent = chatHistory.slice(-6);
    const historyLines: string[] = [];
    for (const m of recent) {
      const label = m.role === 'user' ? (isZh ? '用户' : 'User') : 'AI';
    ...
    const bookContext = isZh
      ? (bookName
        ? `\n<current_book>\n当前阅读的书籍是：《${bookName}》\n${docDescription ? `书籍简介：${docDescription.slice(0, 300)}\n` : ''}</current_book>\n`
        : '')
      : (bookName
        ? `\n<current_book>\nThe current book is: 《${bookName}》\n${docDescription ? `Book description: ${docDescription.slice(0, 300)}\n` : ''}</current_book>\n`
        : '');

    const prompt = isZh
      ? `请分析并输出 JSON。注意：重写 standalone_query 时，如果用户提到“这本书”，请替换为当前书籍的实际名称。`
      : `Please analyze and output JSON. When rewriting standalone_query, replace "this book" with the actual book name.`;
