...
export async function formatterNode(
  ...
  if (state.depth === 0) {
    ...
    const casualPrompt = buildFormatterSystemPrompt(ctx?.memoryContext, ctx?.userProfile);
  ...
  const systemPrompt = buildFormatterSystemPrompt(ctx?.memoryContext, ctx?.userProfile);
