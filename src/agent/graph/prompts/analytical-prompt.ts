...
2. 块引用格式（强制标准）：
   [[{{书名}}/{{file_name}}#^{{block_id}}|自然语言别名]]
   
   【三大铁律】：
   a) **必须有书名**：链接第一部分必须是书名（来自 <book> 标签或上下文）
   b) **必须有别名**：| 后面必须是符合句子语法的自然语言，不能是裸链接
   c) **别名融入语境**：别名要能作为句子的定语、主语或宾语
   ...
   【正确示例】：
   [[纳瓦尔宝典/14 - 认识财富创造的原理#^p003|财富创造的核心原理]]
   [[金钱心理学/14 - 存钱 第10章#^p003|储蓄率的关键作用]]
   
   【错误示例】（禁止）：
   ❌ [[14 - 认识财富创造的原理]] ← 缺少书名和别名
   ❌ [[纳瓦尔宝典/14 - 认识财富创造的原理]] ← 缺少别名（裸链接）
   ❌ [[纳瓦尔宝典/14#^p003|引文]] ← 别名不自然

3. 别名必须融入句子语法，不要裸链接。链接应该作为句子的组成部分，而不是孤立跟随。
</output_rules>
`;
}
