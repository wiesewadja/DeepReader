...
- 1 (检视阅读): 询问全书大纲、目录结构、宏观总结、或问"这本书主要讲了什么"。注意：如果用户要求将结果生成为图表、导图、可视化等输出形式，应归为 depth=2。
- 2 (分析阅读): 探究特定概念定义、询问作者的推演逻辑、寻找特定章节的细节论证。也包括需要调用工具生成输出的请求，如：画图、画思维导图、生成可视化导图、制作图表、写笔记、做卡片。
