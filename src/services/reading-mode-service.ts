...
export class ReadingModeService {
    ...
    /**
     * 引用高亮追踪：filePath → Set<blockId>
     * 记录当前 session 中被用户引用过的 blockId，在章节中加浅红虚线高亮
     * 会话恢复时从 user message 的 quotes 中重建；新对话时清空
     */
    private citedBlocksByFile: Map<string, Set<string>> = new Map();
...
markExcerpt(range: Range): void {
    ...
    /**
     * 公开 API：跳转到指定 blockId + 闪黄色高亮 2s
     * 供引用卡片跳转按钮调用（QuoteManager）
     *
     * @param blockId 块 ID（不含 ^ 前缀）
     * @returns 是否成功跳转（false = blockId 不在当前视图）
     */
    public jumpToBlock(blockId: string): boolean {
        const scrollView = document.querySelector('.deeppdf-reading-mode .markdown-preview-view') as HTMLElement;
        if (!scrollView) {
            serviceLog('[ReadingMode] jumpToBlock: not in reading mode');
            return false;
        }
        const targetElement = scrollView.querySelector(`[id="${blockId}"]`) as HTMLElement;
        if (!targetElement) {
            serviceLog('[ReadingMode] jumpToBlock: target not found', blockId);
            return false;
        }
        this.scrollToElementInColumn(targetElement, scrollView);
        // 黄色闪烁 2s
        targetElement.classList.add('deeppdf-reading-block-flash');
        window.setTimeout(() => {
            targetElement.classList.remove('deeppdf-reading-block-flash');
        }, 2000);
        serviceLog('[ReadingMode] jumpToBlock success:', blockId);
        return true;
    }

    // ============================================================================
    // 引用高亮（用户在阅读中引用某段后，原文加浅红虚线）
    // ============================================================================
    //
    // 定位策略：
    // 1. 首选：DOM 选择器（Obsidian 在标准渲染下会给段落加 id="^blockId" 或 data-block-id）
    // 2. 降级：按引文文本内容匹配段落（适合 DeepReader 自定义阅读模式）
    // 3. 再降级：选最近一个段落
    //
    // 存储：Map<filePath, Map<key, snippet>>，key 优先用 blockId，没有则用文本签名

    private citedHighlightsByFile: Map<string, Map<string, string>> = new Map();

    /**
     * 添加引用高亮
     * @param blockId 段落 ID（可选）
     * @param quotedText 引文文本（用于降级定位 + 持久化）
     * @param filePath 文件路径（可选，默认当前文件）
     * @param range 可选：选区 Range 对象（可获取精准位置）
     */
    public addCitedHighlight(
        blockId: string | undefined,
        quotedText: string,
        filePath?: string,
        range?: Range
    ): void {
        if (!blockId && !quotedText) return;
        const path = filePath || this.currentFile?.path;
        if (!path) return;

        const key = blockId || `text:${quotedText.slice(0, 50)}`;
        if (!this.citedHighlightsByFile.has(path)) {
            this.citedHighlightsByFile.set(path, new Map());
        }
        const fileMap = this.citedHighlightsByFile.get(path)!;
        if (fileMap.has(key)) return;  // 幂等
        fileMap.set(key, quotedText);

        if (this.currentFile?.path === path) {
            this.applyCitedHighlightToDom(key, blockId, quotedText, range);
        }
        serviceLog('[ReadingMode] addCitedHighlight:', path, key, range ? '(with range)' : '(text-fallback)');
    }

    /**
     * 移除引用高亮
     */
    public removeCitedHighlight(blockId: string | undefined, quotedText: string, filePath?: string): void {
        if (!blockId && !quotedText) return;
        const path = filePath || this.currentFile?.path;
        if (!path) return;
        ...
        if (!fileMap) return;
        fileMap.delete(key);
        if (fileMap.size === 0) this.citedHighlightsByFile.delete(path);

        if (this.currentFile?.path === path) {
            this.removeCitedHighlightFromDom(key);
        }
        serviceLog('[ReadingMode] removeCitedHighlight:', path, key);
    }

    /**
     * 批量设置引用高亮（用于会话恢复）
     */
    public setCitedHighlightsByFile(map: Map<string, { blockId?: string; text: string }[]>): void {
        for (const [path, items] of map) {
            ...
            for (const item of items) {
                if (!item.text && !item.blockId) continue;
                const key = item.blockId || `text:${item.text.slice(0, 50)}`;
                if (fileMap.has(key)) continue;
                fileMap.set(key, item.text);
                if (this.currentFile?.path === path) {
                    this.applyCitedHighlightToDom(key, item.blockId, item.text);
                }
            }
        }
        serviceLog('[ReadingMode] setCitedHighlightsByFile: total files:', map.size);
    }

    /**
     * 清除所有引用高亮（用于新对话）
     */
    public clearAllCitedHighlights(): void {
        if (this.currentFile) {
            const fileMap = this.citedHighlightsByFile.get(this.currentFile.path);
            if (fileMap) {
                for (const key of fileMap.keys()) this.removeCitedHighlightFromDom(key);
            }
        }
        this.citedHighlightsByFile.clear();
        serviceLog('[ReadingMode] clearAllCitedHighlights');
    }

    /**
     * 在 DOM 中包裹 inline span（仅高亮选中的文字，不动整个段落）
     *
     * 策略：
     * 1. 优先使用 range 参数：直接 wrapSelectionInSpan
     * 2. 降级（无 range 或 range 失效）：用 textContent 定位 + 选区 API 包裹
     * 3. 都失败：记录日志，不强行高亮段落
     */
    private applyCitedHighlightToDom(
        key: string,
        blockId: string | undefined,
        quotedText: string,
        range?: Range
    ): HTMLElement | null {
        const scrollView = document.querySelector('.deeppdf-reading-mode .markdown-preview-view') as HTMLElement | null;
        if (!scrollView) return null;

        // 策略 1：直接用 range 包裹（实时引用）
        if (range) {
            const span = this.wrapRangeWithSpan(range, key);
            if (span) {
                serviceLog('[ReadingMode] highlight via range');
                return span;
            }
        }

        // 策略 2：用 textContent 定位段落 + 文本位置，构造 Range
        if (quotedText) {
            const needle = quotedText.slice(0, 50).trim();
            if (needle.length > 0) {
                const candidates = scrollView.querySelectorAll('p, h1, h2, h3, h4, h5, h6, li, blockquote');
                for (const c of Array.from(candidates)) {
                    if (!c.textContent?.includes(needle)) continue;
                    // 在该块内查找文本位置
                    const idx = c.textContent.indexOf(needle);
                    if (idx < 0) continue;
                    try {
                        const constructedRange = document.createRange();
                        // textNode 是直接子文本节点
                        ...
                        if (targetNode) {
                            // Range 长度 = 实际选区文本长度（可能超过 50 字符）
                            const rangeLen = Math.min(quotedText.length, (targetNode.textContent?.length || 0) - targetOffset);
                            constructedRange.setStart(targetNode, targetOffset);
                            constructedRange.setEnd(targetNode, targetOffset + rangeLen);
                            const span = this.wrapRangeWithSpan(constructedRange, key);
                            if (span) {
                                serviceLog('[ReadingMode] highlight via text-position');
                                return span;
                            }
                        }
                    } catch (e) {
                        serviceLog('[ReadingMode] text-position highlight failed:', e);
                    }
                }
            }
        }

        serviceLog('[ReadingMode] highlight: failed to find range, skipping');
        return null;
    }

    /**
     * 用 Range.surroundContents 包裹一层 span
     * 如果 range 跨元素边界（surroundContents 抛错），改用 extractContents 模式
     */
    private wrapRangeWithSpan(range: Range, key: string): HTMLElement | null {
        try {
            // 先检查 range 是否有效
            if (range.collapsed) return null;
            const span = document.createElement('span');
            span.className = 'deeppdf-cited-text';
            span.setAttribute('data-cited-key', key);
            range.surroundContents(span);
            return span;
        } catch {
            // surroundContents 在 range 跨元素时会抛错，改用 extractContents 模式
            try {
                const span = document.createElement('span');
                span.className = 'deeppdf-cited-text';
                span.setAttribute('data-cited-key', key);
    ...
    /**
     * 从 DOM 移除高亮（解包 span，恢复原文本节点）
     */
    private removeCitedHighlightFromDom(key: string): void {
        const scrollView = document.querySelector('.deeppdf-reading-mode .markdown-preview-view') as HTMLElement | null;
        if (!scrollView) return;
        const spans = scrollView.querySelectorAll(`span.deeppdf-cited-text[data-cited-key="${CSS.escape(key)}"]`);
        for (const span of Array.from(spans)) {
            // 把 span 的子节点移到 span 父节点中，然后移除 span
