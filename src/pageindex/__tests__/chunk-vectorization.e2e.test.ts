...
function getEmbeddingConfig() {
  ...
  if (envKey) {
    ...
    const config: import("../vault/types.js").EmbeddingOptions = {
      provider,
      apiKey: envKey,
  ...
  try {
    ...
    return {
      ...
      model: embRole.model,
      baseUrl,
    } as import("../vault/types.js").EmbeddingOptions;
  } catch {
    return null;
  }
}
...
describe("E2E Group A: 真实 Vault 数据 + chunker", () => {
  ...
  it("读取真实 .md 文件，splitByBlockIds 正确提取 blockId", async () => {
    const { splitByBlockIds } = await import("../chunker.js");
    const files = fsSync
      .readdirSync(BOOK_DIR)
      .filter((f) => f.endsWith(".md"));
...
describe("E2E Group B: 真实 Embedding API", () => {
  ...
  it("单条 generateEmbedding 返回正确维度向量", async () => {
    ...
    const { generateEmbedding } = await import("../vault/vectors.js");
  ...
  it("批量 generateEmbeddings 返回与输入等长的数组", async () => {
    ...
    const { generateEmbeddings } = await import("../vault/vectors.js");
    const config = getEmbeddingConfig()!;
  ...
  it("相似文本的余弦相似度高于不相似文本", async () => {
    ...
    const { generateEmbeddings } = await import("../vault/vectors.js");
    const { cosineSimilarity } = await import("../core/utils.js");
    const config = getEmbeddingConfig()!;
...
describe("E2E Group C: 完整向量化管线（真实数据 + 真实 API）", () => {
  ...
  it("读取真实 .md → chunk → embed → 写 JSONL → 验证三层产物", async () => {
    ...
    // ── 组装记录 ──
    const vectorRecords: Array<import("../vault/types.js").VectorRecord> = [];
    const chunkTexts: Array<import("../vault/types.js").ChunkTextRecord> = [];
...
describe("E2E Group D: 搜索质量验证（真实向量）", () => {
  ...
  it("cosineSearchJsonl 在 L2 级别返回最相关的段落", async () => {
    ...
    try {
      ...
      const records: Array<import("../vault/types.js").VectorRecord> = texts.map(
        (text, i) => ({
          chunkId: `test_${i}`,
          nodeId: `node${i}`,
          blockIds: [`p${i}`],
  ...
  it("BM25 搜索在已有索引上返回正确结果", async () => {
    ...
    const { searchBM25 } = await import("../bm25.js");
