...
import {
	...
	toggleArchive,
	batchToggleArchive,
} from "../pageindex/archive.js";
import { PAGEINDEX_DIR } from "../pageindex/paths.js";
import { type IndexListItem, type Booklist } from "../types/index.js";
...
import {
	...
	updateCoverHeights,
	addCoverActions,
} from "./library/library-card-builder.js";
...
export class LibraryView extends ItemView {
	...
	async onOpen(): Promise<void> {
		...
		this.coverManager = new CoverManager(this.app, {
			...
			addCoverActions: (coverEl, indexId) => {
				addCoverActions(coverEl, indexId, {
					...
					actions: {
						onHandleZlibDownload: (idx) =>
							this.wereadBridge.handleZlibDownload(idx),
						onHandleLocalAssociate: (idx) =>
							this.wereadBridge.handleLocalAssociate(idx),
		...
		this.wereadBridge = new WereadBridge({
			app: this.app,
			plugin: {
				manifest: { id: this.options.plugin.manifest.id },
			...
			onUpdateCardProgress: (id, progress, status, message) =>
				this.indexLifecycle.updateCardProgress(id, progress, status, message),
		...
		this.multiSelect = new MultiSelectController(
			{
				...
				options: {
					onStartThematicReading: this.options.onStartThematicReading,
				},
				onRenderGrid: () => this.renderGrid(),
				onExitMultiSelect: () => {},
	...
	private getLifecycleCallbacks() {
		return {
			...
			onRefreshIndexes: () =>
				this.options.onRefresh?.() ?? Promise.resolve(undefined),
			onRefreshExternal: () =>
				this.options.onRefresh?.() ?? Promise.resolve([]),
	...
	private render(): void {
		...
		this.injectSvg(addBtn, Icons.add);
		addBtn.title = "添加书籍";
		addBtn.addEventListener("click", () =>
			this.indexLifecycle.handleAddDocument(),
		);

		const thematicBtn = toolbar.createEl("button", {
			cls: "deeppdf-lib-add-btn",
		});
		this.injectSvg(thematicBtn, Icons.booksStacked);
		thematicBtn.title = "主题阅读";
		thematicBtn.addEventListener("click", () =>
			this.multiSelect.toggleMultiSelectMode(),
		);

		this._archiveBtnEl = toolbar.createEl("button", {
			cls: "deeppdf-lib-add-btn",
		});
		this.injectSvg(this._archiveBtnEl, Icons.archive);
		this._archiveBtnEl.title = "显示已归档";
		this._archiveBtnEl.addEventListener("click", () =>
			this.toggleArchiveView(),
		);

		this._filterBtnEl = toolbar.createEl("button", {
			cls: "deeppdf-lib-filter-btn",
		});
	...
	private getBookLastReadTime(index: IndexListItem): number {
		const readingMode = this.options.plugin.readingModeService;
		if (!readingMode) return 0;

		// 获取书籍文件夹路径
		const folderName = this.getDisplayName(index.pdf_name);
		const folderPath = `DeepReader/${folderName}`;

		// 使用 ReadingModeService 的 getBookLastReadTime 方法获取最近阅读时间
		return readingMode.getBookLastReadTime(folderPath);
	}
	...
	private renderGrid(): void {
		...
		if (filterType !== "all") {
			if (filterType === "weread") {
				filtered = filtered.filter((idx) =>
					this.wereadBridge.isWereadLinked(idx),
				);
			} else {
				filtered = filtered.filter(
					(idx) => (idx.fileType || "pdf") === filterType,
				);
			}
		}
		...
		if (filtered.length === 0) {
			const emptyEl = this.gridEl.createDiv({ cls: "deeppdf-lib-empty" });
			const iconEl = emptyEl.createDiv({ cls: "deeppdf-lib-empty-icon" });
			...
			const hasFilter =
				this.searchQuery || filterType !== "all" || filterAuthor;
			emptyEl.createDiv({
				cls: "deeppdf-lib-empty-text",
				text: hasFilter ? "未找到匹配书籍" : "书架空空如也",
			});
			if (!hasFilter) {
				emptyEl.createDiv({
					cls: "deeppdf-lib-empty-hint",
					text: "点击右上角 + 添加第一本书",
				});
			}
			return;
		...
		// 5. 排序
		const sorted = this.filterSort.applySort(filtered, (index) =>
			this.getBookLastReadTime(index),
		);

		// 渲染历史书单卡片
		const history: Booklist[] =
			this.options.plugin.settings?.booklistHistory || [];
		if (
			history.length > 0 &&
			!this.searchQuery &&
			filterType === "all" &&
	...
	private buildBookCard(index: IndexListItem): HTMLElement {
		const ctx: CardBuilderContext = {
			...
			actions: {
				...
				onToggleBookSelection: (id, card) =>
					this.multiSelect.toggleBookSelection(id, card),
				onHandleArchiveBook: (idx) => this.handleArchiveBook(idx),
				onConfirmDelete: (idx) => this.confirmDelete(idx),
				onHandleZlibDownload: (idx) =>
					this.wereadBridge.handleZlibDownload(idx),
	...
	private async handleSelect(index: IndexListItem): Promise<void> {
					...
					try {
						...
						if (await adapter.exists(mappingPath)) {
							...
							const mapping = JSON.parse(raw) as {
								mappings?: Record<string, { deepReaderBookId?: string }>;
							};
		...
		if (READY_STATUSES.has(rawStatus)) {
			const targetFile =
				this.getLastReadChapterFile(index.pdf_name) ||
				this.getFirstChapterFile(index.pdf_name);
			if (targetFile) {
	...
	/**
	 * 使用 DOMParser 安全地将 SVG 字符串注入到父元素中，
	 * 避免 innerHTML XSS 风险。
	 */
	private injectSvg(parent: HTMLElement, svgString: string): void {
	...
	private toggleArchiveView(): void {
		...
		if (this._archiveBtnEl) {
			...
			this._archiveBtnEl.title = this._showArchived
				? "隐藏已归档"
				: "显示已归档";
		}
		this.renderGrid();
	...
	private async handleArchiveBook(index: IndexListItem): Promise<void> {
		try {
			...
			if (newState) {
				...
				new Notice(
					`已取消归档「${this.getDisplayName(index.pdf_name)}」`,
					3000,
				);
			}
			this.renderGrid();
	...
	private async handleBatchArchive(): Promise<void> {
		try {
			...
			const count = await batchToggleArchive(
				...
				try {
					if (index.fileType === "weread") {
						...
						if (adapter) {
							const stateManager = new SyncStateManager(
								adapter,
								this.options.plugin.manifest.id,
							);
					...
					if (this.options.onDeleteIndex) {
						const updatedIndexes =
							await this.options.onDeleteIndex(indexToRemove);
						if (updatedIndexes) {
