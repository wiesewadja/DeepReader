...
export const Icons = {
	...
	volumeX: `<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><line x1="23" y1="9" x2="17" y2="15"/><line x1="17" y1="9" x2="23" y2="15"/></svg>`,
...
export function getIcon(name: IconName, options?: IconOptions): string {
	...
	if (options) {
		...
		// 添加旋转动画
		if (options.spin) {
			svg = svg.replace('<svg', `<svg style="animation: spin 1s linear infinite;"`);
		}
	}
 ...
 * @returns HTML 元素
 */
export function createIconElement(name: IconName, options?: IconOptions): HTMLElement {
 ...
 * @returns SVG HTML 字符串
 */
export function getSpinnerIcon(): string {
	return getIcon('spinner', { spin: true });
}
 ...
 * @returns 对应的图标 SVG
 */
export function getStatusIcon(status: 'loading' | 'success' | 'warning' | 'error'): string {
	switch (status) {
		case 'loading':
