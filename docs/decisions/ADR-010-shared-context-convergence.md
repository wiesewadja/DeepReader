# ADR-010: SharedContext 收敛 — 消除双轨制，确立 State/Context 划界
...
### 方案 A：拆 3 个窄接口（SearchContext / PromptContext / RuntimeContext）
- 优点：接口更窄，职责更清晰
- 缺点：收敛 + 删死字段后 ctx 仅 8 字段，已不算宽；`toolContext`/`recentHistorySummaries` 横跨多个分组，切分不干净；拆分要在 `buildConfigurable` 组装多对象 + 每节点取多键，机械复杂度反增；测试 mock 单对象比多对象更简
- 放弃原因：窄接口可在未来 ctx 再次膨胀时重新评估

### 方案 B：把 mainModel/fastModel/callbacks 也并入 SharedContext 形成 RequestEnvelope
- 优点：统一入口
- 缺点：它们与业务上下文语义不同层，混入会让 ctx 膨胀且模糊职责。顶层留这些键不构成"双轨"（无同名重复）
- 放弃原因：职责边界应保持清晰
...
**风险与缓解：**
- syntopical 跨书 / visualizer abortSignal 生效：`evalBackdoor` 通道先天不注入 `crossBook` 与 `abortSignal`，这两个场景在 ad-hoc 探针里触发不了，**以代码审查兜底**——signal 路径单一无分支（`buildConfigurable → SharedContext.abortSignal → ctx.abortSignal`），守卫修复是纯类型修正（`.app`→`.vault?.app`）、节点内部逻辑未动，且 syntopical 此前恒 early-return 故"恢复执行"等价于首次启用，下游问题属预存潜伏而非本次引入。留待该路径 deemed 高频或再次改动时补。

**架构约束：**
- 新增业务上下文字段必须加入 `SharedContext`，不得再在 `configurable` 顶层塞键。
...
- ⏸ **syntopical 跨书 / visualizer abortSignal 生效**：有意搁置，以代码审查兜底（见上文风险与缓解）。